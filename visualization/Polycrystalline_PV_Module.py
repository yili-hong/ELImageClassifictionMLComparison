...
def metric_combine(file_name_1, file_name_2, file_name_3, file_name_4, file_name_5):
    ...
    PRECISION = list(itertools.chain(final_result_1['PRECISION'], final_result_2['PRECISION'], final_result_3['PRECISION'], final_result_4['PRECISION'], final_result_5['PRECISION'], final_result_5['PRECISION']))
...
poly_data = {'Functional': 920, 'Mildly D': 178, 'Moderately D': 50, 'Severely D': 402}
plot_bar_with_percentage(data = poly_data, xlabel_list = xlabel, total = returnSum(poly_data), figure_size = (7, 7), bar_color = 'purple',
                        file_name = 'poly_description.pdf')

# Read the csv result file of both ML and DL models
read_path = 'csv_result/' 
os.chdir(read_path)
poly_vgg19_metrics = metric_combine(file_name_1 = 'poly_vgg19_pred010.csv', file_name_2 = 'poly_vgg19_pred1020.csv', 
                                   file_name_3 = 'poly_vgg19_pred2030.csv', file_name_4 = 'poly_vgg19_pred3040.csv',
                                   file_name_5 = 'poly_vgg19_pred4050.csv')

poly_resnet50_metrics = metric_combine(file_name_1 = 'poly_resnet50_pred010.csv', file_name_2 = 'poly_resnet50_pred1020.csv', 
                                   file_name_3 = 'poly_resnet50_pred2030.csv', file_name_4 = 'poly_resnet50_pred3040.csv',
                                   file_name_5 = 'poly_resnet50_pred4050.csv')

poly_log = ml_pred_true_split('poly_log.csv')
poly_svm = ml_pred_true_split('poly_svm.csv')
poly_rf = ml_pred_true_split('poly_rf.csv')
poly_log_result = ml_metrics_calculation_overall(poly_log[0], poly_log[1])
poly_svm_result = ml_metrics_calculation_overall(poly_svm[0], poly_svm[1])
poly_rf_result = ml_metrics_calculation_overall(poly_rf[0], poly_rf[1])

# Standard Deviation
std_poly = std_calculation(poly_vgg19_metrics, poly_resnet50_metrics, poly_log_result, poly_svm_result, poly_rf_result)
poly_std = std_reorganization(std_poly)

# Radar plot of Standard deviation
categories =  ['MCC', 'Accuracy', 'Precision', 'Balanced Accuracy', 'F1-Macro']
categories = [*categories, categories[0]]
VGG19  = poly_std['VGG19']
ResNet50 = poly_std['ResNet50']
Log =  poly_std['Log']
SVM =  poly_std['SVM']
RF  = poly_std['RF']
VGG19 = [*VGG19, VGG19[0]]
...
#             i = i + 1
ax.set_xticklabels([])
plt.savefig("Poly_std.pdf", bbox_inches = 'tight')
# plt.show()

# poly median 
# random seed = 7 - logistic 
poly_log_acc = pd.Series(poly_log_result['ACC'])
# random seed = 12 - SVM 
poly_svm_acc = pd.Series(poly_svm_result['ACC'])
# random seed = 27 - RF 
poly_rf_acc = pd.Series(poly_rf_result['ACC'])
# random seed = 5 - VGG19 
pd.Series(poly_vgg19_metrics['ACC'])
# random seed = 15 - ResNet50
pd.Series(poly_resnet50_metrics['ACC'])
poly_vgg19_median = []
for key in poly_vgg19_metrics.keys():
    poly_vgg19_median.append(poly_vgg19_metrics[key][5])
poly_resnet50_median = []
for key in poly_resnet50_metrics.keys():
    poly_resnet50_median.append(poly_resnet50_metrics[key][15])
poly_log_median = []
for key in poly_log_result.keys():
    poly_log_median.append(poly_log_result[key][7])    
poly_svm_median = []
for key in poly_svm_result.keys():
    poly_svm_median.append(poly_svm_result[key][12])
poly_rf_median = []
for key in poly_rf_result.keys():
    poly_rf_median.append(poly_rf_result[key][27])
...
median_poly = median_metrics(poly_vgg19_median, poly_resnet50_median, poly_log_median, poly_svm_median, poly_rf_median)
median_poly_model = model_dividence_bst_model(median_poly)
poly_med = median_format_reorganize_data(median_poly)

# Radar plot of Median
categories =  ['MCC', 'Accuracy', 'Precision', 'Balanced Accuracy', 'F1-Macro']
categories = [*categories, categories[0]]
VGG19  = poly_med['VGG19']
ResNet50 = poly_med['ResNet50']
Log =  poly_med['Log']
SVM =  poly_med['SVM']
RF  = poly_med['RF']
VGG19 = [*VGG19, VGG19[0]]
...
for label, angle in zip(ax.get_xticklabels(), angles):
    if i <= 5:
        if i == 2 or i == 1:
            ...
            i = i + 1
ax.set_xticklabels([])
plt.savefig("Poly_med.pdf", bbox_inches = 'tight')
# plt.show()

# Loss of VGG19, random selected
vgg19_bst_loss = loss_extract(data_csv = "poly_vgg19_pred4050.csv", which_run = 8, one_stage = False)
# Loss of ResNet50, random selected
resnet50_bst_loss = loss_extract(data_csv = "poly_resnet50_pred010.csv", which_run = 2, one_stage = True)
loss_plot([*vgg19_bst_loss['train_loss_s1'], *vgg19_bst_loss['train_loss_s2']], [*vgg19_bst_loss['val_loss_s1'], *vgg19_bst_loss['val_loss_s2']], epoch_list = [i+1 for i in range(0, 100)], 
          plot_title = 'Loss_Mono_BST_VGG19.pdf')


# Confusion Matrix of Random Forest
poly_rf = ml_pred_true_split('poly_rf.csv')
poly_rf_result = ml_metrics_calculation_overall(poly_rf[1], poly_rf[0])
# 30 is the replicate selected among 50 replicates
confusion_matrix_plot(poly_rf[0][30], poly_rf[1][30], cm_name = 'cm_rf_poly_bst.pdf')

# Confusion Matrix of VGG19
poly_vgg19_bst = pd.read_csv("poly_vgg19_pred4050.csv", header = None)
#vgg19_bst_poly =  output_pred(input_string = 'poly_vgg19_pred4050.csv')
data_output = poly_vgg19_bst.T
output = data_output.drop(0, axis = 0)
output = output.set_axis(list(poly_vgg19_bst.iloc[:,0]), axis=1)
pred_output = output.pred_output
poly_vgg19_pred = output_pred(pred_output)
confusion_matrix_plot(poly_vgg19_pred['true'][8], poly_vgg19_pred['pred'][8], cm_name = 'cm_vgg19_poly_bst.pdf')

# Confusion Matrix of ResNet50
poly_resnet50_bst = pd.read_csv("poly_resnet50_pred010.csv", header = None)
data_output = poly_resnet50_bst.T
output = data_output.drop(0, axis = 0)
output = output.set_axis(list(poly_resnet50_bst.iloc[:,0]), axis=1)
pred_output = output.pred_output
poly_vgg19_pred = output_pred(pred_output)
confusion_matrix_plot(poly_vgg19_pred['true'][2], poly_vgg19_pred['pred'][2], cm_name = 'cm_resnet50_poly_bst.pdf')
